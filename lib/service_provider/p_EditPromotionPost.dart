...
class p_EditPromotionPost extends StatefulWidget {
  ...
  const p_EditPromotionPost({Key? key, required this.docId}) : super(key: key);
...
class _p_EditPromotionPostState extends State<p_EditPromotionPost> {
  ...
  bool isEditing = true;
  double PDiscountPercentage = 0.0; // ✅ Declare at class level
  ...
  Future<void> _fetchPData() async {
    DocumentSnapshot snapshot =
    await _firestore.collection('promotion').doc(widget.docId).get();
    ...
    if (snapshot.exists) {
      ...
      setState(() {
        ...
        // priceController.text = (data['PPrice'] ?? '').toString();
        priceController.text = (data['PPrice'] as num?)?.toInt().toString() ?? '0';
        APriceController.text = (data['PAPrice'] as num?)?.toInt().toString() ?? '0';
        PDiscountPercentage = (data['PDiscountPercentage'] as num?)?.toDouble() ?? 0.0;
        _imageUrls = List<String>.from(data['PImage'] ?? []);
        ...
        entries = (data['PDescriptions'] as List<dynamic>?)
  ...
  Future<void> updatePromotionPost() async {
    ...
    try {
      // Add to Firestore
      await _firestore.collection('promotion').doc(widget.docId).update({
        // 'userId': userId,
        // 'SPname': spName,
      ...
      ReusableSnackBar(
        context,
        "Promotion Post Updated Successfully!",
