...
class p_ResubmitApplication extends StatefulWidget {
  @override
  _p_ResubmitApplicationState createState() => _p_ResubmitApplicationState();
}

class _p_ResubmitApplicationState extends State<p_ResubmitApplication> {
  ...
  Future<void> _saveProfileChanges() async {
    ...
    setState(() {
      ...
      _validateCertificate(); // Validate certificate before saving
    });
    ...
    if (!isNameValid || !isPhoneValid || !isDobValid || selectedGender == null || selectedStates.isEmpty || selectedExpertiseFields.isEmpty || bioError != null || certificateError != null) {
      Navigator.pop(context); // ✅ Close loading dialog before returning
      ReusableSnackBar(
        context,
    ...
    try {
      User? user = _auth.currentUser;
      if (user != null) {
        DocumentReference providerRef = _firestore.collection(
            'service_providers').doc(user.uid);

        // Retrieve current resubmission count
        DocumentSnapshot providerDoc = await providerRef.get();
        int currentResubmissionCount = providerDoc.exists
            ? (providerDoc['resubmissionCount'] ?? 0)
            : 0;
        ...
        await providerRef.update({
          ...
          // Increment resubmission attempts
          'status': "Pending",
        });
        ...
        await EmailService.sendEmail(
            ...
            "FixMate - Application Successfully Resubmitted!",
            "Dear ${nameController.text.trim()},\n\n"
                "Your application has been resubmitted for verification. This is attempt ${currentResubmissionCount +
                1}/3.\n\n"
                "Currently, your application is in pending status. You will receive another email once your registration is approved.\n\n"
                "Thank you!\n\n"
                "Best regards,\nFixMate Team"
        );
        ...
        ReusableSnackBar(
          context,
          "Application is successfully resubmitted!",
      ...
      if (context.mounted) {
        Navigator.pop(context); // ✅ Ensure loading dialog is closed only once
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Resubmit Application", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.white,)),
